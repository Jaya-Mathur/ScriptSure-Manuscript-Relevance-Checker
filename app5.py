...
def extract_sections(text):
    ...
    abstract = re.search(r'(?s)(?<=\bABSTRACT\b)(.*?)(?=\bINTRODUCTION\b)', text)
    introduction = re.search(r'(?s)(?<=\bINTRODUCTION\b)(.*?)(?=\bLITERATURE SURVEY\b)', text)
    problem_statement = re.search(r'(?s)(?<=\bPROBLEM STATEMENT AND OBJECTIVES\b)(.*?)(?=\bPROPOSED METHOD\b)', text)
    proposed_method = re.search(r'(?s)(?<=\bPROPOSED METHOD\b)(.*?)(?=\bRESULTS AND DISCUSSION\b)', text)
    conclusion = re.search(r'(?s)(?<=\bCONCLUSION AND FUTURE ENHANCEMENTS\b)(.*?)(?=\bREFERENCES\b)', text)
...
#             weight = section_weights.get(section_names[i], 1.0) * section_weights.get(section_names[j], 1.0)
...
if uploaded_file:
    with st.spinner("Processing the PDF..."):
        try:
            ...
            st.subheader("Semantic Similarity Scores (scaled to 0-5):")
